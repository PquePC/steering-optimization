...
| [28](28-portable-resume-bundle.md) | A run must survive losing its volume | RunPod keeps forcing a move to a host the network volume cannot follow, and the volume also failed mid-run. There is an export path and no import path; `scan.jsonl` alone is 34 min of GPU | **yes — before the next run** |
| [31](31-execute-every-path-before-any-run.md) | Execute every M3 path offline before any further run | Nine defects so far and **not one was found by a test** — two by a run failing, one after spending money, three by accident. All lived in paths nothing had executed. Build a fake-GPU harness so the whole pipeline runs on a laptop | **BUILD NOW — BLOCKS ALL RUNS** |
| [30](30-m2-m3-seam-audit.md) | Audit the whole M2/M3 seam | M3 reuses M2's hook, generation, judge transport and I/O. Five wiring defects found so far, three of them by accident. The general form is a value M3 owns that some M2 path reads from somewhere else | absorbed into [31](31-execute-every-path-before-any-run.md) |
| [29](29-judge-free-probe.md) | Measure the mid band directly, judge-free | `e6` reads the concept's mass at the FIRST answer token, so it only fires at collapse — and it gates which cells ever reach `d2`. Every `d2` in the last run was at L47+, and the band M1 found the dissociation in was never measured | BUILT — awaiting the run |
